...
const BookNowModal = (props) => {
  ...
  console.log(cityTimezones.findFromCityStateProvince('Los Angeles United States of America'));
  ...
  >
    <Box sx={style} component="form" noValidate autoComplete="off">
      <Grid container spacing={1} p={2}
        ...
        <Grid item xs={4} flex={1}>
          <Typography fontSize={14}>
            <LocalAirportIcon sx={{ fontSize: 18 }} color="primary" /> {flight.cityFrom} to {flight.cityTo}
          </Typography>
        </Grid>
        <Grid item xs={4} flex={1}>
          <Typography fontSize={14}>
            <CalendarMonthIcon sx={{ fontSize: 18 }} color="primary" />  {moment(flight.local_departure).format("DD MMM YYYY")} - {moment(flight.utc_arrival).format("DD MMM YYYY")}
          </Typography>
        </Grid>
        <Grid item xs={4} flex={1}>
          <Typography fontSize={14}>
            ...
            <Accordion expanded={expanded === id} onChange={handleExpand(id)}>
              ...
              >
                ...
                <Typography sx={{ color: 'text.secondary' }}>
                  {moment(utc_departure).format("MMM DD YYYY kk:mm")} - {moment(utc_arrival).format("MMM DD YYYY")}</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Typography>
      ...
      <Button
        ...
        onClick={() => window.open(`https://www.kiwi.com/en/booking?token=${bookingItem.booking_token}`, '_blank')}
      >
        Visit Kiwi
      </Button>
    </Box>
...
const RouteItem = ({ route, fontSize = 10 }) => {
  ...
  return <>
    ...
    {route.map(routeItem => {
      const mins = moment.utc(moment(routeItem.local_arrival).diff(moment(routeItem.local_departure)))

      console.log(mins)

      console.log(routeItem)
      return <Grid container spacing={1} p={1}
        alignItems="center">
        <Grid item xs={3}>
          <Typography
            fontWeight={600}
          ...
          <Typography fontSize={fontSize}>
            <FlightTakeoffIcon sx={{ fontSize }} color="primary" /> Depart - {routeItem.cityFrom}
          </Typography>
        </Grid>
        <Grid
          item
        ...
        <Grid item xs={3}>
          ...
          <Typography fontSize={fontSize}>
            <FlightLandIcon sx={{ fontSize }} color="primary" /> Arrive - {routeItem.cityTo}
          </Typography>
