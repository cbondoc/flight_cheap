...
import { Grid, Item, Container, Typography, Box, Button } from "@mui/material";
...
function BookingItem({ booking }) {
  ...
  const [mins] = useState(
    moment.utc(moment(booking.utc_arrival).diff(moment(booking.utc_departure))).format("mm")
  );
  ...
  return (
    <Container sx={{mb: 5}}>
      ...
      >
        {booking.status &&
          <Grid sx={{mb: 2}}>
            <Typography fontSize={12} fontWeight={600} color={booking.status === 'Confirmed' ? '#5cab4c' : 'red'}>
        ...
        <Grid container spacing={1}>
          <Grid item xs={2}>
            ...
            <Typography fontSize={12}>
              <FlightTakeoffIcon sx={{fontSize: 12}} color="primary"/> Depart - {booking.cityFrom}
            </Typography>
          </Grid>
          <Grid item xs={1}>
          ...
          <Grid item xs={2}>
            ...
            <Typography fontSize={12}>
            <FlightLandIcon sx={{fontSize: 12}} color="primary"/> Arrive - {booking.cityTo}
            </Typography>
          </Grid>
          <Grid item xs={1}>
            <Typography>
          ...
          <Grid item xs={2}>
            <Typography>
              {booking.booking_token.slice(0, 5)}
            </Typography>
